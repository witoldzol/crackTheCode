...
public class MyHashTable<K, V> {
    ...
    void insert(K key, V value) {
        ...
        if ( isResizeRequired() ) resizeList();
    }
    ...
    public void remove(K key) {
        ...
        if (head == null) return;
            //set the next node as a head
        else if (prev == null) bucketList.set(index, head.next);
        //if node is in middle or last, it has the same 'treatment'
        prev.next = head.next;
    ...
    private int getBucketIndex(K key) {
        return key.hashCode() % listCapacity;
    }
